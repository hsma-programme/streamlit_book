...
1. We import streamlit into the script
2. We use some Streamlit commands to add titles and instructions for the user.
3. We create some inputs so the user can alter parameters such as the run length, the number of resources, the number of runs, and the default length of different activities.
4. We join these inputs up with our DES code by passing them in to the g class.
5. We display the relevant output with Streamlit commands.
...
st.write("In this discrete event simulation, patients arrive and are seen by a nurse in the order of arrival, then leave the system.")
```
...
### The Patient, Model and Trial Classes
...
For example, here we just have a Pandas dataframe summarising the results - so we'll display that using the `st.dataframe` command.
...
:::{.callout-tip}
Text can be displayed with `st.write()` or `st.markdown()`.

Pandas dataframes are displayed with `st.table()` or `st.dataframe()`.
