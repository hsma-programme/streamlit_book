...
With minimal additional changes, we can improve the app so the code does not run without us first pressing a button.

Most of our code is unchanged - it is only the final few lines where we
...
:::{.callout-tip}
An alternative way to approach this would be with the [`st.form`](https://docs.streamlit.io/develop/api-reference/execution-flow/st.form) function.
:::
...
We just make a minor additional tweak to include a `with` clause referring to `st.spinner`.
